...
const Address = ({ onSelectAddress }) => {
  ...
  return (
    <div className="shadow-sm border p-3  lg:p-6 border-gray-300 rounded-sm lg:ml-20  lg:w-[700px]">
      {!showAddressForm && (
        <h3 className=" sm:text-lg font-semibold text-black mb-4">
          Shipping Address
        </h3>
      )}
      {!showAddressForm && addresses.length > 0 && (
        <ul className="space-y-2">
          {addresses.map((address) => (
            <li
              key={address._id}
              className="flex items-start justify-between p-2 cursor-pointer border-b pb-2"
            >
              <div
